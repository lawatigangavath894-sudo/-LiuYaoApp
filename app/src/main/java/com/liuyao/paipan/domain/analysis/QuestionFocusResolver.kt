...
object QuestionFocusResolver {
    fun resolve(question: String): String {
        ...
        return when {
            q.isBlank() -> "未命名占事的主结果"
            q.contains("录取") || q.contains("录不录") -> "录取 / 不录取"
            q.contains("入职") || q.contains("上岗") || q.contains("offer", ignoreCase = true) -> "正式入职 / 不入职"
            q.contains("能不能过") || q.contains("能过") || q.contains("过吗") || q.contains("通过") -> "通过 / 不通过"
            q.contains("联系") || q.contains("消息") || q.contains("回复") -> "有联系消息 / 无联系消息"
